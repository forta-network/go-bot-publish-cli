...
func Run(ctx context.Context, cfg *Params) error {
	...
	if !strings.EqualFold(toAddr.Hex(), cfg.To) {
		return fmt.Errorf("invalid address %s", cfg.To)
	}

	res, err := r.Contracts().AgentRegTx.SafeTransferFrom(opts, common.HexToAddress(agt.Owner), toAddr, utils.AgentHexToBigInt(agentID))
	if err != nil {
		return err
	}
